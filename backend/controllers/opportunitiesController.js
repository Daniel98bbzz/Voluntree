const opportunityRepository = require('../repository/opportunityRep');
...
const getAllOpportunities = async (req, res, next) => {
  try {
    const allOpportunities = await opportunityRepository.getAllOpportunities();
    if (allOpportunities.length === 0) {
      logger.info('No Opportunities found.');
      return res.status(404).send('No Opportunities found.');
    }
...
const getItemByName = async (req, res, next) => {
  try {
    ...
    const item = await opportunityRepository.getItemByName(req.params.itemName);
    if (!item) {
...
const createItem = async (req, res, next) => {
  ...
  // }
  try {
    const existingItem = await opportunityRepository.getItemByName(req.body.item_name);
    if (existingItem) {
      logger.info('A item with this name already exists.');
      throw new ConflictError('A item with this name already exists.');
    }y
    const newItem = await opportunityRepository.createItem(req.body);
...
const updateItem = async (req, res, next) => {
  ...
  if (req.paramitemName) {
    const findItem = await opportunityRepository.getItemByName(req.params.itemName);
    if(!findItem) {
  ...
  if (req.body.item_name) {
    const exsist = await opportunityRepository.getItemByName(req.body.item_name);
    if (exsist) {
  ...
  try {
    const updatedItem = await opportunityRepository.updateItem(req.params.itemName, req.body);
...
const deleteItem = async (req, res, next) => {
  try {
    const deletedItem = await opportunityRepository.deleteItem(req.params.itemName);
    if (!deletedItem) {
